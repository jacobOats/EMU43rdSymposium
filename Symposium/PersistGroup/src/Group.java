...
public class Group
{
    ...
    private double persistDensity;    //density of persiting opinion 
    ...
    public Group(int size, int iterations, double density, int seed, double persistDensity, double noise)
    {
    ...
    /************************************************************************/
     ...
     private void changeNeighborhood(int i, int j, int x) {
    	 if(x == PLUS) { //set neighborhood to plus
    		 if(array[(i+1)%size][j] != PERSIST)
    			 array[(i+1)%size][j] = PLUS;
    		 if(array[(i-1+size)%size][j] != PERSIST)
    			 array[(i-1+size)%size][j] = PLUS;
    		 if(array[i][(j+1)%size] != PERSIST)
    			 array[i][(j+1)%size] = PLUS;
    		 if(array[i][(j-1+size)%size] != PERSIST)
